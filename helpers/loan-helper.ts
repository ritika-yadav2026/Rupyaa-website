...
import { getLoanTypeDisplayName, getTotalPayable } from "@/utils/common-helper";
...
/**
 * GET /user/stage `context.offerStatus` values that mean “under review” while stage may still be OFFERINGS.
 * Extend when backend documents more pending-review values.
 */
export function isPendingOfferStatusForUnderReview(offerStatus?: string | null): boolean {
  if (offerStatus == null || typeof offerStatus !== "string") return false;
  const u = offerStatus.trim().toUpperCase().replace(/[\s-]+/g, "_");
  return (
    u === "PENDING_REVIEW" ||
    u === "IN_REVIEW" ||
    u === "UNDER_REVIEW" ||
    u === "APPLICATION_PENDING" ||
    u === "PENDING_IN_REVIEW"
  );
}

/** Route wizard to register “under review” when offerings are held for review (native parity). */
export function shouldRouteOfferingsToUnderReviewFromStage(
  stage: string,
  offerStatus?: string | null
): boolean {
  const normalized = stage.replace(/[-\s]/g, "_").toUpperCase();
  return normalized === "OFFERINGS" && isPendingOfferStatusForUnderReview(offerStatus);
}
...
export function normalizeLoanStatus(loan: Loan | null | undefined): string {
...
export type LoanStatusColorVariant = 'error' | 'success' | 'warning' | 'neutral';

const ERROR_STATUSES = ['npa', 'expired', 'overdue'];
const SUCCESS_STATUSES = ['paid', 'waivered', 'foreclosed'];
const WARNING_STATUSES = ['sanctioned', 'disbursed'];

/** Maps a loan/payment status string to a pill color variant (mobile parity). */
export function getLoanStatusColorVariant(status: string | null | undefined): LoanStatusColorVariant {
  if (!status || typeof status !== 'string') return 'neutral';
  const s = status.trim().toLowerCase();
  if (ERROR_STATUSES.includes(s)) return 'error';
  if (SUCCESS_STATUSES.includes(s)) return 'success';
  if (WARNING_STATUSES.includes(s)) return 'warning';
  return 'neutral';
}
