...
export function getManualUploadStatementRangeDescription(now: Date = new Date()): React.ReactNode {
  ...
  return (
    <>
      ...
      <span className="font-semibold">{rangeText}</span>
      {' '}salary-account statement
    </>
  );
}

export function renderStatementRangeLabels() : React.ReactNode {
  ...
  return [
    '(Last 3 months ',
    <span key="statement-range" className="font-semibold">
      including {statementRangeLabels.startLabel} – {statementRangeLabels.endLabel}
    </span>,
    ')',
  ];
};